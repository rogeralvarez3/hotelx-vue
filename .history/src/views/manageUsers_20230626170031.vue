          ...
          <v-simple-table dense>
            <thead>
              <tr>
                ...
                <th>{{ $store.getters.getLang("delete") }}</th>
              </tr>
            </thead>
            <tbody>
...
export default {
  ...
  methods: {
    getData: function (token) {
      const mv = this;
      mv.$store.dispatch("getData", { table: "users", token: token });
    },
    save: function () {
      const mv = this;
    ...
    removeUser: function (id) {
      ...
      mv.$swal
        .fire({
          icon: "warning",
          title: mv.$options.filters.capital(
            mv.$store.getters.getLang("confirm delete")
          ),
          text: mv.$options.filters.capital(
            mv.$store.getters.getLang("user will be deleted permanently")
          ),
          showCancelButton: true,
          confirmButtonText: mv.$options.filters.capital(
            mv.$store.getters.getLang("yes, delete")
          ),
        })
        .then((res) => {
          if (res.value) {
