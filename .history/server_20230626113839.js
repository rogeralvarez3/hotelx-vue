...
app.post("/scrap", async (req, res) => {
  ...
  if (req.body.params) {
    Object.keys(req.body.params).forEach((paramKey) => {
      url.searchParams.append(paramKey, req.body.params[paramKey]);
    });
  }

  let site = await fetch(url, {
    headers: {
      cookie: {
        SOCS: "CAISOQgEEitib3FfaWRlbnRpdHlmcm9udGVuZHVpc2VydmVyXzIwMjMwNjIwLjA1X3AwGgZlcy00MTkgAhoGCICq46QG",
      },
    },
  });
  res.send(site);
